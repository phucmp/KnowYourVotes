    ...
    <div class="container">
        <checkbox-svg-map v-model="selectedLocations" :map="customUSA" @click="onClick">
          <template v-slot:after>
            <text v-for="(state, index) in states" :key="index" style="font-size: 16px;">
              <textPath :xlink:href="'#'+state" text-anchor="middle" startOffset="50%" stroke="black" fill="none">{{state.toUpperCase()}}</textPath>
            </text>
          </template>
        </checkbox-svg-map>
    </div>
